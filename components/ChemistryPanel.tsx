  ...
  return (
    <div className="card-gradient rounded-xl p-3 text-left">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs font-bold uppercase tracking-wider text-slate-400">🧪 Química</span>
      ...
      <div className="flex flex-wrap gap-1.5 text-[10px]">
        {topClubs.map(([club, n]) => (
          <span key={club} className="badge-celeste px-2 py-0.5 rounded-full">🔗 {club} x{n}</span>
        ))}
        {nationLinks.length > 0 && (
          <span className="px-2 py-0.5 rounded-full bg-yellow-500/15 text-yellow-300 border border-yellow-500/20">🌎 Nacionalidad x{nationLinks.length}</span>
        )}
        {outOfPosition > 0 && (
          <span className="px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/20">⚠️ {outOfPosition} fuera de posición natural</span>
        )}
